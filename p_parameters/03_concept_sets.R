...
ds_more_split_conceptsets <- c("CPRD")
ds_split_conceptsets <- c("SNDS", "ARS", "TEST")
ds_less_split_conceptsets <- c("BIFAP")

CONCEPTSETS_to_be_split <- if (thisdatasource %in% ds_more_split_conceptsets) {
  c("DP_COVCARDIOCEREBROVAS", "DP_DILIMOST", "DP_ANALGESIC", "DP_IMMUNOSUPRESSANTSONLYPAN")
} else if (thisdatasource %in% ds_split_conceptsets) {
  c("DP_COVCARDIOCEREBROVAS", "DP_DILIMOST", "DP_ANALGESIC")
} else if (thisdatasource %in% ds_less_split_conceptsets) c("DP_COVCARDIOCEREBROVAS") else c()

numbers_split <- if (thisdatasource %in% ds_more_split_conceptsets) c(20, 20, 20, 20) else c(15, 15, 15)

# File_SAFETY_VAC_BRIDGE is the name of the input file (also used in 06_variable_lists)
File_SAFETY_VAC_BRIDGE <- paste0(thisdir, "/p_parameters/archive_parameters/SAFETY-VAC_O2_BRIDGE_04Oct24.xlsx")

#  OUT_codelist: it is the dataframe containing the codelist itself (temporary: it is removed later)
OUT_codelist <- data.table::fread(paste0(thisdir, "/p_parameters/archive_parameters/20241022_ROC18_codelist_with procedures_at_20250319.csv"))
PROC_codelist <- data.table::fread(paste0(thisdir, "/p_parameters/archive_parameters/additional procedures codes.csv"))
OUT_codelist <- data.table::rbindlist(list(OUT_codelist, PROC_codelist))
...
VAR_list <- data.table::as.data.table(readxl::read_excel(File_SAFETY_VAC_BRIDGE, sheet = "Variables"))
VAR_list <- VAR_list[rowSums(VAR_list[, c("AESI", "COV", "Algorithm", "Algorithm_input")]) > 0]
VAR_list <- VAR_list[!(Algorithm & !Algorithm_input), .(Varname = Concept_id_codelist)]
VAR_list <- VAR_list[!is.na(Varname), ]
...
domain_list <- as.data.table(readxl::read_excel(File_SAFETY_VAC_BRIDGE, sheet = "Variables"))
domain_list <- domain_list[rowSums(domain_list[, c("AESI", "COV", "Algorithm_input")]) > 0]
domain_list <- domain_list[!(Algorithm), ]
domain_list <- domain_list[, .(Varname, Type_codelist)]
domain_list <- domain_list[!is.na(Type_codelist), ]
domain_list <- domain_list[, Type_codelist := fcase(Type_codelist == "diagnosis", "Diagnosis",
                                                    Type_codelist == "drug", "Medicines",
                                                    Type_codelist == "procedure", "Procedures")]
...
DRUG_codelist <- DRUG_codelist[Drug_proxie %in% unlist(VAR_list), ]
rm(VAR_list)
...
for (concept in names(concept_set_codes_our_study)) {
  concept_set_domains[[concept]] = domain_list[Varname %in% concept | paste0(Varname, "_narrow") %in% concept | paste0(Varname, "_possible") %in% concept, Type_codelist]
}
...
if (!is.null(CONCEPTSETS_to_be_split)) {
  ...
  df_CONCEPTSETS_to_be_split[, group := cut(seq_along(code), unique(numbers_split), labels = FALSE),
  ...
  df_CONCEPTSETS_to_be_split_NA <- df_CONCEPTSETS_to_be_split_NA[CJ(coding_system = coding_system,
                                                                    ...
                                                                    group = seq_len(unique(numbers_split)), unique = TRUE),
  ...
  df_concept_set_domains_to_be_split <- df_concept_set_domains_to_be_split[CJ(concept = concept,
                                                                              group = seq_len(unique(numbers_split)),
                                                                              unique = TRUE),
                                                                           on = .(concept)]
