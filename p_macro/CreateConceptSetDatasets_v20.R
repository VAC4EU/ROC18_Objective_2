...
CreateConceptSetDatasets <- function(dataset, codvar, datevar, EAVtables, EAVattributes, dateformat, rename_col,
  ...
  for (dom in used_domains) {
    ...
    for (df2 in dataset_in_dom) {
      ...
      coln.int64 <- names(which(sapply(used_df, bit64::is.integer64)))
      if (length(coln.int64) > 0L)
        used_df[, c(coln.int64) := lapply(.SD, as.numeric), .SDcols = coln.int64]
      ...
      if(!missing(rename_col)){
        ...
        for (elem in names(rename_col)) {
          data <- get(elem)
          if (elem %in% names(used_df)) {
            setnames(used_df, elem, data[[dom]][[df2]])
          }
        }
      }
