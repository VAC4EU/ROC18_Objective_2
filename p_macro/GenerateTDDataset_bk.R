...
GenerateTDDataset <- function(datasets,
  ...
  # TO DO: replace datasets[[i]] with a copy, so that the original datasets are not modified
  ...
  for (i in 1:2) {
   ...
   if (nrow(to_add) > 0){
    ...
    datasets[[i]][,(paste0("last_day_observed_",i)) := max(get(end_d_vars[i]), na.rm = TRUE),  by = c(UoO_vars[i]) ]
  ...
  if (start_d_vars[2] != start_d_vars[1]){
    ...
    setnames(datasets[[2]],start_d_vars[2],start_d_vars[1])
  }
  ...
  #########################
  # merge the two datasets (in case there is a date observed by both) and sort
  ...
  dt_final <- merge(datasets[[1]][,..tokeep1],datasets[[2]][,..tokeep2], by = c(UoO_var_final, start_d_var_final), all = T, allow.cartesian = T)
  ...
  # 
#   ##############
#   # collapse values of all the variables of dt_final observed on the same day: collapse all variables to their max by UoO_var_final and start_d_var_final, ignoring missing values. the action is a bit convoluted to address the situation when the values of a variable in the first stratum of UoO_var_final and start_d_var_final are all missing: data.table does not know how to set the type of the resulting variable and this needs to be stored before entering the procedure
# 
#   cols_to_collapse <- setdiff(names(dt_final), c(UoO_var_final,start_d_var_final))
#   
#   View(dt_final)
#   
#   # Get the types of the columns before entering the group operation
#   column_types <- sapply(dt_final[, ..cols_to_collapse], class)
#   
#   collapsed_dt <- dt_final[, lapply(.SD, function(x) {
#     
#     # get the column index
#     
# #    i <- which(sapply(.SD, function(col) identical(col, x)))
#     # Initialize i to 0 (or NULL) to store the index of the column
#     i <- 0
#     
#     # Use a loop to find the index of x in .SD
...
#     
#     # Get the column name
#     col_name <- colnames(.SD)[i]
#     print(col_name)
# 
#     # Print the column being processed and group info for debugging
#     print(paste("Processing column:", col_name))
#     print(paste("Current group: UoO_var_final =", get(UoO_var_final), ", start_d_var_final =", get(start_d_var_final)))
#     flush.console()
#     # Check if the current group (x) contains only NA values
#     if (all(is.na(x))) {
#       print("All values in this column are NA for this group.")
#       print(column_types[i][1])
#       
#       # Return NA of the appropriate type based on the column type determined earlier
#       if (column_types[i][1] == "integer") return(NA_integer_)
#       if (column_types[i][1] == "numeric") return(NA_real_)
#       if (column_types[i][1] == "character") return(NA_character_)
#       if (column_types[i][1] == "logical") return(NA)
#       if (column_types[i][1] == "Date") return(NA_Date_)
#       if (column_types[i][1] == "IDate") return(as.IDate(NA))
#     } else {
#       # Calculate max, ignoring NA values
#       print(paste("assigned value",max(x, na.rm = TRUE)))
