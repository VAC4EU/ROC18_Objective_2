...
listdatasetsRData <- c()
listdatasets <- c("D3_components_flare_TD_E_GRAVES_AESI")

# dates variables 

listdates <- list()

temp <- as.data.table(readxl::read_excel((paste0(thisdir, "/", paste0("D3_components_flare_TD_",immdis), ".xlsx") )))
booleanvar <- names(temp)[grep("^bool", names(temp))]
listdates[[paste0("D3_components_flare_TD_",immdis)]] <- setdiff(names(temp),c(booleanvar,"person_id"))
